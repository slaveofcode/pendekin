...
const code_generator = require(`${lib_root}/code_generator`);
...
describe("Code Generator", () => {
  it("Should be able to generate unique code", () => {
    ...
    assert.isTrue(
      uniqueCode.length > 0,
      "Unique code length is bigger than zero"
    );
  });
  ...
  it("Should be able to generate unique code on certain length", () => {
    ...
    assert.isTrue(
      generatedCode1.length > 0,
      "Generated Code length is bigger than zero"
    );
    assert.isTrue(
      generatedCode1.length >= lengthFour,
      `Generated Code length is equal or more than ${lengthFour}`
    );
    ...
    assert.isTrue(
      generatedCode2.length > 0,
      "Generated Code length is bigger than zero"
    );
    assert.isTrue(
      generatedCode2.length >= lengthSix,
      `Generated Code length is equal or more than ${lengthSix}`
    );
    ...
    assert.isTrue(
      generatedCode3.length > 0,
      "Generated Code length is bigger than zero"
    );
    assert.isTrue(
      generatedCode3.length >= lengthEight,
      `Generated Code length is equal or more than ${lengthEight}`
    );
  });
  ...
  it("Should be able to generate bulk unique code at certain length", () => {
    ...
    assert.isTrue(
      generatedBulks.length === bulkLength,
      `Bulk generated code length is equal to ${bulkLength}`
    );

    generatedBulks.forEach(code => {
      expect(code).to.be.a("string");
      assert.isTrue(
        code.length >= lengthCode,
        `Generated Code length is equal or more than ${lengthCode}`
      );
    });
  });
});
